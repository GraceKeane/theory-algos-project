...
// Summary:     A program that executes the sha-512 of an input
...
/* Determine the endianess of your machine. Adapted from: https://developer.ibm.com/technologies/systems/articles/au-endianc */
const int _i = 1;
...
/* Enum is essentially a flag for keeping track of input message / padding */
enum Status {
    READ, PAD, END
};

/* Section 4.2.3 of the Secure Hash Standard (SHS) - defining 64 bit words */
const WORD K[] = {
...
/* Section 6.4 of the Secure Hash Standard - preprocessing 
...
int next_block(FILE *f, union Block *M, enum Status *S, uint64_t *nobits){
    ...
    if(*S == END){
        ...
        *nobits = *nobits + (8 * nobytes); 
        ...
        if(nobytes == 128){
            /* Successfully reading 64 bytes from f */
        } else if(nobytes < 120){
            /* If has enough room for padding */
        ...
        /* Swap the byte order of the words for little endian machine. */
        if (islilend())
            for (int i = 0; i < 16; i++) 
...
** Hash values calculated with corresponding message. e.g. H_1 uses M_2 to calculate H_2. 
...
/* Function that executes the SHA-512 computation */
...
int main(int argc, char *argv[]){  
    ...
    /* Initial hash value (described in Section 5.3.5) - Assigning H to be a local variable */
    WORD H[] = {
    ...
    if (argv[1] == NULL){
        ...
        if(option == 1){
            ...
            /* User wants to input a file name */
        ...
        switch (c) {
            ...
            case 'f': /* Allow user to choose file to be calculated */
                ...
                /* If invalid file input - file error handling */
                if(!f){
